...
import { LiquidityPoolKeys, CpmmPoolInfoInterface, CpmmConfigInfoInterface } from "./type";
...
export const getPoolInfo = async ({ connection, poolId }: { connection: Connection; poolId: PublicKey }) => {
  const poolState = await connection.getAccountInfo(poolId);
  if (!poolState) throw new Error(`pool not found: ${poolId.toBase58()}`);
  const poolInfo = CpmmPoolInfoLayout.decode(poolState.data) as CpmmPoolInfoInterface;
  ...
  if (!configState) throw new Error(`pool config not found: ${poolInfo.configId.toBase58()}`);
  const config = CpmmConfigInfoLayout.decode(configState.data) as CpmmConfigInfoInterface;
