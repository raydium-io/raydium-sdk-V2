...
import { BIT_PRECISION, BN_ONE, BN_ZERO, EXTENSION_TICKARRAY_BITMAP_SIZE, LOG_B_2_X32, LOG_B_P_ERR_MARGIN_LOWER_X64, LOG_B_P_ERR_MARGIN_UPPER_X64, MAX_SQRT_PRICE_X64, MAX_TICK, MIN_SQRT_PRICE_X64, MIN_TICK, Q64, TICK_ARRAY_BITMAP_SIZE, TICK_ARRAY_SIZE, TICK_TO_SQRT_PRICE_FACTORS } from "./constants";
...
export class TickArrayBitmapUtil {
  static findTickArrayStartIndex({
    ...
    const exItemArrayCount = 8 * 8

    const poolBitmapInitBit: number[] = []

    if (findInfo.type === 'all') {
      for (let i = 0; i < EXTENSION_TICKARRAY_BITMAP_SIZE; i++) {
        for (let k = 0; k < 8; k++) {
          for (let w = 0; w < 8; w++) {
            const arrIndex = i * exItemArrayCount + (7 - k) * 8 + w
            for (let j = 0; j < 8; j++) {
              if ((tickArrayBitmap.negativeTickArrayBitmap[arrIndex] & (1 << j)) !== 0) {
                poolBitmapInitBit.push(this.bitmapIndexToStartIndex({ type: 'l', index: (i * exItemArrayCount + k * 8 + w) * 8 + j, tickSpacing }))
              }
            }
          }
        }
      ...
      for (let i = 0; i < poolBitmap.length; i++) {
        for (let j = 0; j < 8; j++) {
          if ((poolBitmap[i] & (1 << j)) !== 0) {
            poolBitmapInitBit.push(this.bitmapIndexToStartIndex({ type: 'm', index: i * 8 + j, tickSpacing }))
          }
        }
      }

      for (let i = 0; i < tickArrayBitmap.positiveTickArrayBitmap.length; i++) {
        for (let j = 0; j < 8; j++) {
          if ((tickArrayBitmap.positiveTickArrayBitmap[i] & (1 << j)) !== 0) {
            poolBitmapInitBit.push(this.bitmapIndexToStartIndex({ type: 'r', index: i * 8 + j, tickSpacing }))
          }
        }
      }
        ...
        for (let i = 0; i < EXTENSION_TICKARRAY_BITMAP_SIZE; i++) {
          for (let k = 0; k < 8; k++) {
            for (let w = 0; w < 8; w++) {
              ...
              for (let j = 0; j < 8; j++) {
                if ((tickArrayBitmap.negativeTickArrayBitmap[arrIndex] & (1 << j)) !== 0) {
                  const v = this.bitmapIndexToStartIndex({ type: 'l', index: (i * exItemArrayCount + k * 8 + w) * 8 + j, tickSpacing })
                  if (v >= startTickIndex) {
                    poolBitmapInitBit.push(v)
                    if (poolBitmapInitBit.length === findInfo.count) return poolBitmapInitBit
                  }
                }
              }
            }
      ...
      if (startIndex < 512) {
        for (let i = 0; i < poolBitmap.length; i++) {
          for (let j = 0; j < 8; j++) {
            if ((poolBitmap[i] & (1 << j)) !== 0) {
              const v = this.bitmapIndexToStartIndex({ type: 'm', index: i * 8 + j, tickSpacing })
              if (v >= startTickIndex) {
                poolBitmapInitBit.push(v)
                if (poolBitmapInitBit.length === findInfo.count) return poolBitmapInitBit
              }
      ...
      for (let i = 0; i < tickArrayBitmap.positiveTickArrayBitmap.length; i++) {
        for (let j = 0; j < 8; j++) {
          if ((tickArrayBitmap.positiveTickArrayBitmap[i] & (1 << j)) !== 0) {
            const v = this.bitmapIndexToStartIndex({ type: 'r', index: i * 8 + j, tickSpacing })
            if (v >= startTickIndex) {
              poolBitmapInitBit.push(v)
              if (poolBitmapInitBit.length === findInfo.count) return poolBitmapInitBit
            }
      ...
      if (startIndex >= 512) {
        for (let i = tickArrayBitmap.positiveTickArrayBitmap.length - 1; i >= 0; i--) {
          for (let j = 8 - 1; j >= 0; j--) {
            if ((tickArrayBitmap.positiveTickArrayBitmap[i] & (1 << j)) !== 0) {
              const v = this.bitmapIndexToStartIndex({ type: 'r', index: i * 8 + j, tickSpacing })
              if (v <= startTickIndex) {
      ...
      if (startIndex >= -512) {
        for (let i = poolBitmap.length - 1; i >= 0; i--) {
          for (let j = 8 - 1; j >= 0; j--) {
            if ((poolBitmap[i] & (1 << j)) !== 0) {
              const v = this.bitmapIndexToStartIndex({ type: 'm', index: i * 8 + j, tickSpacing })
              if (v <= startTickIndex) {
      ...
      for (let i = EXTENSION_TICKARRAY_BITMAP_SIZE - 1; i >= 0; i--) {
        for (let k = 7; k >= 0; k--) {
          for (let w = 7; w >= 0; w--) {
            const arrIndex = i * exItemArrayCount + (7 - k) * 8 + w
            for (let j = 7; j >= 0; j--) {
  ...
  static bitmapIndexToStartIndex({ type, index, tickSpacing }: { type: 'l' | 'm' | 'r', index: number, tickSpacing: number }): number {
    if (type === 'm') {
      const startIndex = TICK_ARRAY_BITMAP_SIZE
      return (index - startIndex) * tickSpacing * TICK_ARRAY_SIZE
    } else if (type === 'l') {
      const startIndex = TICK_ARRAY_BITMAP_SIZE + 2
      return (index * -1 - startIndex) * tickSpacing * TICK_ARRAY_SIZE
    } else if (type === 'r') {
      const startIndex = TICK_ARRAY_BITMAP_SIZE
      return (index + startIndex) * tickSpacing * TICK_ARRAY_SIZE
    } else {
...
export class TickArrayUtil {
  ...
  static nextInitalizedTick({
    data, currentTickIndex, tickSpacing, zeroForOne
  }: {
    data: ReturnType<typeof TickArrayLayout.decode>,
...
export class TickUtil {
  ...
  static matchLimitOrder({ tick, swapAmount, swapDirectionZeroForOne, isBaseInput, feeRate, isFeeOnInput }: {
    ...
    if (isBaseInput) {
      ...
      result.amountOut = this.getLimitOrderOutput({ amountIn: result.amountIn, tick: tick.tick, zeroForOne: swapDirectionZeroForOne })

      if (result.amountOut.gt(totalUnfilledAmount)) {
        result.amountOut = totalUnfilledAmount
        result.amountIn = this.getLimitOrderInput({ amountOut: totalUnfilledAmount, tick: tick.tick, zeroForOne: !swapDirectionZeroForOne })

        if (isFeeOnInput) {
          result.ammFeeAmount = mulDivCeil(result.amountIn, new BN(feeRate), FEE_RATE_DENOMINATOR_VALUE.sub(new BN(feeRate)))
        }
      }
    } else {
      ...
      if (isFeeOnInput) {
        ...
        result.amountOut = BN.min(mulDivCeil(netOutput, FEE_RATE_DENOMINATOR_VALUE, FEE_RATE_DENOMINATOR_VALUE.sub(new BN(feeRate))), totalUnfilledAmount)
      }

      result.amountIn = this.getLimitOrderInput({ amountOut: result.amountOut, tick: tick.tick, zeroForOne: !swapDirectionZeroForOne })
    ...
    if (amountOutContinueToConsume.gt(BN_ZERO)) {
      ...
      tick.partFilledOrdersRemaining = tick.partFilledOrdersRemaining.add(tick.ordersAmount.sub(amountOutContinueToConsume))
